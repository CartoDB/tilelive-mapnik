        ...
        .then((resource) => {
            ...
            try {
                ...
                map.render(image, options, (err, image) => {
                    ...
                    if (metatile.tiles.length > 1) {
                        renderStats.render = Math.round((Date.now() - renderStartTime) / metatile.tiles.length);
                        return this._sliceMetatile(image, options, metatile, renderStats, callback);
                    }

                    renderStats.render = Date.now() - renderStartTime;

                    this._encodeSingleTile(image, options, metatile, renderStats, callback);
                });
            } catch(err) {
                this._mapPool.release(map);
                return callback(err);
    ...
    meta.tiles.forEach(c => {
        ...
        this._getImage(source_image, options, x, y, (err, image) => {
            ...
            tiles[key] = {
                ...
                stats: Object.assign(stats, { encode: Date.now() - encodeStartTime }, source_image.get_metrics())
            };
