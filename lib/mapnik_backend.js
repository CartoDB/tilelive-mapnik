...
MapnikSource.prototype._createPool = function(xml) {
    ...
    const maxPoolSize = this._uri.query.poolSize || cpusNumber;
    const maxWaitingClients = this._uri.query.poolMaxWaitingClients || (maxPoolSize * cpusNumber);

    const options = {
        max: maxPoolSize,
        maxWaitingClients: maxWaitingClients > 64 ? maxWaitingClients : 64
    };
