...
MapnikSource.prototype.getGrid = function (z, x, y, callback) {
    ...
    if (!this._info || !this._info.interactivity_fields || !this._info.interactivity_layer) {
        if (!this._info) {
...
MapnikSource.prototype._open = function (callback) {
    ...
    // TODO: move this to normalize URI
    this._internal_cache = this._uri.query.internal_cache;
    this._autoLoadFonts = this._uri.query.autoLoadFonts;
    this._base = this._uri.query.base;
    this._uri.query.metatile = +this._uri.query.metatile;
    this._uri.query.resolution = +this._uri.query.resolution;
    this._uri.query.bufferSize = +this._uri.query.bufferSize;
    this._uri.query.tileSize = +this._uri.query.tileSize;
...
MapnikSource.prototype._sliceMetatile = function (source_image, options, meta, stats, callback) {
    ...
    meta.tiles.forEach(c => {
        ...
        getImage(this, source_image, options, x, y, (err, image) => {
            tile_num++;
            if (err) {
                if (!err_num) {
                    return callback(err);
...
MapnikSource.prototype._encodeSingleTile = function (source_image, options, meta, stats, callback) {
    ...
    getImage(this, source_image, options, 0, 0, function (err, image) {
        if (err) {
            return callback(err);
        }
...
function getImage(source, image, options, x, y, callback) {
    ...
    view.isSolid(function (err, solid, pixel) {
        ...
        if (solid && source.solidCache[pixel_key]) {
            return callback(null, source.solidCache[pixel_key]);
        }
