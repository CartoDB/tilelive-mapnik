...
// TODO: remove internal cache.
// see: https://github.com/mapbox/tilelive-mapnik/issues/47
//      https://github.com/mapbox/tilelive-mapnik/issues/59
//      https://github.com/tilemill-project/tilemill/issues/1893
const cache = {};
...
MapnikSource.prototype.getTile = function (z, x, y, callback) {
    ...
    this._tileCache.get(key, (err, tile, headers, stats) => {
        if (err) {
            return callback(err);
        }
...
MapnikSource.prototype.getGrid = function (z, x, y, callback) {
    ...
    this._tileCache.get(key, (err, grid, headers, stats) => {
        if (err) {
            return callback(err);
        }
...
MapnikSource.prototype._createMetatileCache = function (options) {
    ...
    function cacheGenerator (cacheInput) {
        ...
        self._renderMetatile(options, metatile, (err, tiles) => {
            if (err) {
...
MapnikSource.prototype._encodeSingleTile = function (source_image, options, meta, stats, callback) {
    ...
    this._getImage(source_image, options, 0, 0, (err, image) => {
        if (err) {
            return callback(err);
        }
