...
module.exports = function normalizeURI (uri) {
    ...
    else uri.query.internal_cache = asBool(uri.query.internal_cache);
    ...
    else uri.query.autoLoadFonts = asBool(uri.query.autoLoadFonts);
    ...
    uri.query.metatileCache.deleteOnHit = uri.query.metatileCache.hasOwnProperty('deleteOnHit') ?
        asBool(uri.query.metatileCache.deleteOnHit) : false;

    if (typeof uri.query.metrics === 'undefined')  uri.query.metrics = false;
    else uri.query.metrics =  asBool(uri.query.metrics);
    return uri;
};
